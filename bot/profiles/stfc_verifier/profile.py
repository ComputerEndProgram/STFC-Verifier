...
class STFCVerifierProfile(VerificationProfile):
    ...
    async def assign_roles(
        ...
        config: GuildConfig,
    ) -> tuple[list[str], Optional[RankConfirmationView]]:
        feedback = []
        ...
        guild = member.guild
        member_role = guild.get_role(config.member_role_id) if config.member_role_id and guild else None
        commodore_role = guild.get_role(config.commodore_role_id) if config.commodore_role_id and guild else None
        admiral_role = guild.get_role(config.admiral_role_id) if config.admiral_role_id and guild else None

        rank_tier = get_rank_tier(getattr(player_data, "rank", None))

        if not member_role:
            feedback.append(f"❌ Member role {config.member_role_id} not found - cannot assign ranks")
            log.error(f"[WIZARD] Member role {config.member_role_id} not found for {member.id}")
        else:
            try:
            ...
            if player_data.alliance_tag:
                ...
                if alliance_role:
                    try:
                        ...
                        bot.store.update_user_alliance_role_id(member.id, alliance_role.id)
    ...
    async def handle_update(
        ...
        if old_rank != player_data.rank:
            ...
            confirm_embed.add_field(
                name="Player", value=f"{member.mention} ({player_data.username})", inline=False
            )
            ...
            log_msg = await bot.post_to_log_channel(guild.id, embed=confirm_embed, view=confirmation_view)
            if log_msg:
