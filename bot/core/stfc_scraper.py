...
class PlayerData:
    ...
    def __repr__(self):
        alliance_str = (
            f"alliance=[{self.alliance_tag}]" if self.alliance_tag else "alliance=None"
        )
        rank_str = f", rank={self.rank}" if self.rank else ""
        return f"PlayerData(id={self.player_id}, name={self.username}, level={self.level}, {alliance_str}, server={self.server}{rank_str})"
...
class STFCProScraper:
    ...
    @staticmethod
    def fetch_player_data(player_id: str) -> PlayerData | None:
        ...
        for base_url in [STFCProScraper.BASE_URL_PRO, STFCProScraper.BASE_URL_WTF]:
            ...
            if not title_tag or not meta_desc:
                log.debug(
                    f"Could not find required metadata for player {player_id} at {base_url}"
                )
                continue
            ...
            if not desc_match:
                log.debug(
                    f"Could not parse alliance/server from description: {description}"
                )
                continue
            ...
            # Extract rank from HTML body (Agent, Operative, Premier, Commodore, Admiral)
            rank_match = re.search(
                r">(Agent|Operative|Premier|Commodore|Admiral)</span>", response.text
            )
        ...
        # Both URLs failed
        log.warning(
            f"Could not fetch player data for {player_id} from stfc.pro or stfc.wtf"
        )
        return None
